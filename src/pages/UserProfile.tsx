...
export const UserProfile: React.FC<UserProfileProps> = ({ userId, onBack, onChat, onBookClick }) => {
  ...
  const userListings = books.filter(b => user.listings.includes(b.id));
  // Mock wishlist items for now since we don't have full book objects in wishlist array in mock data
  // In a real app, we'd fetch these
  const wishlistItems = books.slice(0, 2); 
      ...
      <header className="px-4 py-6 flex flex-col items-center relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-sm mb-6">
          <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-3 flex flex-col items-center relative overflow-hidden group">
            ...
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-1">Trust Score</span>
            <span className="text-2xl font-bold text-white font-display">{user.trustScore}</span>
          </div>

          <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-3 flex flex-col items-center relative overflow-hidden group">
            <div className="absolute inset-0 bg-gradient-to-br from-amber-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            <span className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-1">Rating</span>
          ...
          <div className="space-y-3">
            {user.wishlist.length > 0 ? (
              user.wishlist.map((item, i) => (
                <div key={i} className="bg-slate-900/50 border border-white/5 rounded-xl p-4 flex items-center gap-3">
          ...
          <div className="mt-8 bg-slate-900/50 border border-white/5 rounded-2xl overflow-hidden">
            <button className="w-full p-4 flex items-center gap-4 hover:bg-white/5 transition-colors border-b border-white/5">
              ...
              <div className="flex-1 text-left">
                <span className="block text-sm font-bold text-slate-200">My Wallet</span>
              </div>
            ...
            <button className="w-full p-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
              ...
              <div className="flex-1 text-left">
                <span className="block text-sm font-bold text-slate-200">Settings</span>
              </div>
              <ChevronRight className="w-5 h-5 text-slate-600" />
            </button>
          </div>
