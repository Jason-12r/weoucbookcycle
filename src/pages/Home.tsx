...
export const Home: React.FC<HomeProps> = ({ onBookClick }) => {
  ...
  const categories = [
    { id: 'Used Books', label: 'Used', icon: Book, color: 'from-orange-400 to-pink-500' },
    { id: 'Textbook', label: 'Textbook', icon: GraduationCap, color: 'from-blue-400 to-indigo-500' },
    { id: 'Non-Textbook', label: 'Non-textbook', icon: Coffee, color: 'from-emerald-400 to-teal-500' },
    { id: 'New Book', label: 'New', icon: Sparkles, color: 'from-purple-400 to-fuchsia-500' },
    { id: 'QR Code for English Books', label: 'QR Code', icon: QrCode, color: 'from-slate-400 to-slate-600' },
  ];
    ...
    <div className="flex flex-col gap-6 pb-24">
      ...
      <header className="sticky top-0 z-30 px-4 py-3 bg-slate-900/80 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="relative w-10 h-10 rounded-full bg-slate-800 hover:bg-slate-700 transition-all flex items-center justify-center border border-white/5">
            <Bell className="w-5 h-5 text-slate-300" />
            <span className="absolute top-2.5 right-2.5 h-2 w-2 rounded-full bg-red-500 border border-slate-800"></span>
          </button>
        </div>
        ...
        <div className="relative flex items-center w-full group">
          ...
          <div className="absolute inset-y-0 right-0 flex items-center pr-2">
            <button className="p-2 rounded-xl hover:bg-white/5 text-slate-400 hover:text-indigo-400 transition-colors">
          ...
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 snap-x snap-mandatory">
            <div className="snap-center shrink-0 w-[90%] relative rounded-3xl overflow-hidden shadow-2xl shadow-black/50 group cursor-pointer h-48 border border-white/10">
              ...
              <div className="absolute inset-0 z-20 p-6 flex flex-col justify-end">
                ...
                <h2 className="text-2xl font-bold text-white mb-1.5 font-display tracking-tight">Summer Reading List</h2>
                <p className="text-sm text-slate-200 font-medium opacity-90">Curated sets up to 20% off this week.</p>
              </div>
            </div>
            ...
            <div className="snap-center shrink-0 w-[90%] relative rounded-3xl overflow-hidden shadow-2xl shadow-black/50 group cursor-pointer h-48 border border-white/10">
              ...
              <div className="absolute inset-0 z-20 p-6 flex flex-col justify-end">
                ...
                <h2 className="text-2xl font-bold text-white mb-1.5 font-display tracking-tight">Verified Traders</h2>
                <p className="text-sm text-slate-200 font-medium opacity-90">High trust scores & fast shipping.</p>
              </div>
            </div>
          </div>
        </section>
      ...
      {!searchQuery && (
        <section className="px-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-slate-100 font-display">Categories</h3>
            {selectedCategory && (
              <button 
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {!searchQuery && (
            <div className="flex gap-1 bg-slate-800 p-1 rounded-lg border border-white/5">
              ...
              <button className="p-1.5 rounded-md text-slate-500 hover:text-white transition-colors">
