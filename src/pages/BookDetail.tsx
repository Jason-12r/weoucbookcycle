...
export const BookDetail: React.FC<BookDetailProps> = ({ id, onBack, onChat, onSellerClick }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 relative overflow-y-auto no-scrollbar pb-24">
      ...
      <header className="fixed top-0 left-0 w-full z-50 px-4 py-4 flex items-center justify-between max-w-md mx-auto left-0 right-0">
        <button onClick={onBack} className="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white hover:bg-white/10 transition-colors">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <div className="flex gap-3">
          <button className="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 text-white hover:bg-white/10 transition-colors">
      ...
      <div className="px-4 -mt-2 relative z-10">
        <div className="relative bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6 shadow-lg overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between border-t border-white/5 pt-4">
            <div className="flex flex-col">
              <span className="text-xs text-slate-400 mb-1">Price</span>
            ...
            <div className="flex gap-2">
              ...
              <span className="px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300 text-xs font-medium">Original</span>
            </div>
          </div>
        </div>
        ...
        {/* Seller Info */}
        <div className="mt-6">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">Seller Reputation</h3>
          <div 
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-400 text-[10px] font-bold tracking-wide uppercase">Verified</span>
                </div>
                <div className="text-xs text-slate-400 mt-0.5">Joined {seller.joinDate} • {seller.sales} Sales</div>
              </div>
            </div>
            <div className="flex flex-col items-end">
              ...
              <span className="text-[10px] text-slate-500 mt-1">Trust Score</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 mb-4 px-2">
          <h3 className="text-lg font-bold text-white mb-2 font-display">Description</h3>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-slate-950/90 backdrop-blur-md border-t border-white/5 px-6 py-4 pb-8 z-40 max-w-md mx-auto left-0 right-0">
        <div className="flex items-center gap-4">
          ...
          <button className="flex-[2] h-12 rounded-xl flex items-center justify-center gap-2 text-white font-bold bg-indigo-600 shadow-[0_0_15px_rgba(79,70,229,0.3)] hover:bg-indigo-500 active:scale-95 transition-all duration-200">
            <span>Buy Now</span>
