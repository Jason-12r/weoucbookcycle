...
import { messages, chats, books, users, ChatMessage } from '../data/mockData';
...
export const ChatDetail: React.FC<ChatDetailProps> = ({ id, onBack }) => {
  ...
  useEffect(() => {
    setLocalMessages(messages[id] || messages['1']);
  }, [id]);
    ...
    <div className="flex flex-col h-full bg-slate-950 relative overflow-hidden">
      ...
      <header className="sticky top-0 z-50 px-4 pt-4 pb-3 flex items-center justify-between bg-slate-900/60 backdrop-blur-xl border-b border-white/5 shadow-lg">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-indigo-400 font-medium tracking-wide">Online</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/30 px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1 transition-all backdrop-blur-md hover:shadow-[0_0_15px_rgba(99,102,241,0.3)]">
            <span>Buy Now</span>
      ...
      <main className="flex-grow z-10 overflow-y-auto no-scrollbar pt-4 pb-4 px-4 flex flex-col gap-6">
        <div className="flex justify-center my-2">
          <span className="px-3 py-1 rounded-full bg-slate-800/40 backdrop-blur-md text-[10px] text-slate-400 font-medium border border-white/5 shadow-sm">Today 10:23 AM</span>
        </div>
        ...
        {book && (
          <div className="flex justify-start w-full">
            <div className="bg-slate-800/40 backdrop-blur-xl p-3 rounded-3xl rounded-tl-sm max-w-[85%] border border-white/10 shadow-xl">
              ...
              <p className="text-xs text-slate-300 font-light">Hi! I saw you're interested in this book. It's still available.</p>
            </div>
          </div>
        )}

        {localMessages.slice(1).map((msg) => (
      ...
      <div className="z-50 px-4 py-3 pb-8 bg-slate-900/80 backdrop-blur-2xl border-t border-white/5 mx-auto w-full shadow-[0_-10px_40px_rgba(0,0,0,0.3)]">
        ...
        <div className="flex gap-2 mb-3 overflow-x-auto no-scrollbar pb-1">
          {['Can you ship today?', 'Is the price negotiable?', 'More photos?'].map((text, i) => (
            <button 
              key={i} 
        ...
        <div className="flex items-end gap-2">
          <button className="w-10 h-10 flex items-center justify-center rounded-full bg-white/5 border border-white/10 text-slate-400 hover:text-indigo-400 hover:bg-white/10 transition-all flex-shrink-0 backdrop-blur-sm active:scale-95">
