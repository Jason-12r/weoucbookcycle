...
export const BookCard: React.FC<BookCardProps> = ({ book, onClick }) => {
  ...
  return (
    <div 
      ...
      className="bg-slate-800/40 backdrop-blur-md rounded-2xl p-3 flex flex-col border border-white/5 active:scale-[0.98] transition-all duration-200 hover:bg-slate-800/60 hover:shadow-lg hover:shadow-indigo-500/5 group cursor-pointer"
    >
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden mb-3 bg-slate-800 shadow-inner">
        ...
        <button 
          onClick={handleShare}
          className="absolute top-2 right-2 p-1.5 rounded-full bg-black/40 backdrop-blur-md text-white hover:bg-white/20 transition-colors border border-white/10 z-10 opacity-0 group-hover:opacity-100 focus:opacity-100"
        >
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 bg-emerald-900/30 px-1.5 py-0.5 rounded border border-emerald-500/20">
            ...
            <span className="text-[10px] font-bold text-emerald-200">Verified</span>
          </div>
        </div>
      </div>
